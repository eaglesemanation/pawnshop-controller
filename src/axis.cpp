...
Axis::Axis(Axis&& src) : motor(std::move(src.motor)),
...
void Axis::move(const double newPos, const double scaling) {
    ...
    setSpeed(v0);
    std::thread stepping(&Axis::step, this, static_cast<uint32_t>(std::ceil(S / stepLength)));
    const auto dt = 1ms;
    const double dv = a * dt.count() / (1000ms).count();
    // Acceleration
    while (dir * (getPosition() - oldPos) < SAccel && dir * (vMax - getSpeed()) > 0 ) {
    ...
    std::cout << "Cur pos: " << getPosition() << ", diff: " << dir * (newPos - getPosition()) << ", SAccel: " << SAccel << std::endl;
    // Deceleration
    while (dir * (newPos - getPosition()) > 0 && dir * (getSpeed() - v0) > 0) {
        setSpeed(getSpeed() - dv);
        std::this_thread::sleep_for(dt);
    }
    std::cout << "Cur pos: " << getPosition() << ", diff: " << dir * (newPos - getPosition()) << std::endl;
    stepping.join();
}
...
void Axis::step(uint32_t steps) {
    for (uint32_t i = 0; i < steps; i++) {
        ...
        if (motor.getDirection() == Motor::NEGATIVE && negative.has_value() && negative.value()) {
            break;
        }
        motor.step();
